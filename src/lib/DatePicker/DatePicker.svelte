...
<dialog id="date-picker" bind:this={ref}>
  <DatePickerHeader bind:currentDate={currentDate} bind:isReverse />
  <DatePickerWeekDays />
  <div class="days-container" class:reverse={isReverse}>
    ...
    <section class="days" transition:slide={{ duration: 5e2, axis: "x" }}>
        ...
        {#each Array.from({ length: lastDate }) as _, index}
          {@const parseCurrentDate = (inc = 1) => new Date(useDate(currentDate).year, useDate(currentDate).month, index + inc)}
          ...
          <DateItem {isOlder} {isCurrent} {index} clickHandler={isClickable ? () => updateDate(index + 1) : void(0)} />
        {/each}
